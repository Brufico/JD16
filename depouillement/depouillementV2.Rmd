...
#### En considérant uniquement les diplômés sur le marché du travail (i.e en activité/ création d'entreprise ou en recherche d'emploi)
...
La proportion des diplômés de la promotion qui sont en poste hors de France est donc de `r ptable[2,3]`


### Situation par grandes zones géographiques {-}
...
Cette distribution par pays est très peu exploitable du fait que l'effectif correspondant à chaque pays est très réduit. En particulier, de nomreux pays comptent un seul diplômé sur leur sol, et  aucune conclusion générale ne peut donc être formulée. Tout au plus cette distribution permet-elle de mieux comprendre la précédente.
```{r}
...
La rémunération (ensemble des répondants)
-----------------------------------------
...
REM : question 71 Disposez-vous d'avantages en nature ? => aucune réponse)
...
### la rémunération totale annuelle brute hors éléments variables (primes, commissions...) (Question 69)

Cette question n'est exploitée qu'à titre documentaire.


```{r}
res <- getresult("votre_emploi_remuneration_brut")
...
```{r}
plot2 <- plot + labs(x = "Partie fixe de la rémunération annuelle brute", y = "effectif")
plot2
```
...
```{r}
# Test ANOVA
# garder uniquement les valeurs non manquantes de la rémunération
datadf=filter(emploidf, !is.na(votre_emploi_remuneration_total))
# sélectionner les filières d'effectif suffisant'
fileff <- datadf %>%
        group_by(filiere) %>%
        summarise(n = n()) %>%
        filter(n >= 2)
...
datadf <- datadf %>%
        filter(filiere %in% fileff[[1]] )

# faire le test
test <- oneway.test(votre_emploi_remuneration_total ~ filiere , 
                    data = datadf, na.action=na.omit)

```
La rémunération ne dépend pas significativement de la filière (p-valeur = `r test$p.valeur`)




La recherche de l'emploi occupé
-------------------------------
...
```
La proportion d'emplois issus de l'expérience professionnelle acquise à l'Ipag (stage, alternance...) est de `r x1 / n1`. Cette proportion est significativement inférieure à celle obtenue l'an dernier, c.a.d. `r x2 / n2`. (p-valeur : `r test$p.value`)
